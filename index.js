...
app.get("/", function (req, res) {
        res.json({
            output: "Wellcome to Hall Booking Api"
        });
...
app.post("/createRoom", function (req, res) {
    ...
    else{res.status(400).json({ output: 'Please specify No of seats for Room'})};
    if(req.body.amenities){room.amenities = req.body.amenities} 
    else{res.status(400).json({ output: 'Please specify all Amenities for Room in Array format'})};
    if(req.body.price){room.price = req.body.price} 
    else{res.status(400).json({ output: 'Please specify price per hour for Room'})};
    rooms.push(room);
    roomNo++;
...
app.post("/createBooking", function (req, res) {
    ...
    if(availableRooms.length == 0){res.status(400).json(
        { output: 'Sorry! No Rooms Available for Date and Time'}
    )}
   else{
